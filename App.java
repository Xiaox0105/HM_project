...
public class App {
    ...
    static SimpleDateFormat myFormat = new SimpleDateFormat("dd/MM/yyyy"); // Construct a SimpleDateFormat for parsing String to Date
    ...
    public static void main(String[] args) throws ParseException {
        ...
        first =  myFormat.parse("01/01/2021"); // set the first day of the year to 01/01/2021, which corresponds to the 0th index of the arrays. 
        ...
        while (true) {
            ...
            } else if (inp.equalsIgnoreCase("Manager")) { // Enter the Manager Mode, check the total income within a date range.
                System.out.println("(1) View expected income for a range of dates \n(2) Cancel an existing reservation");
                int inp2 = input.nextInt(); 

                if (inp2 == 1) {
                    manager.totalIncome(DR, KR, KS, PS); // Call the totalIncome method in the Manager class to calculate the total hotel income in the date range
    ...
    public static int getStartdate() throws ParseException {
        ...
        int start = (int)daysBetween(LB,first); // Call the daysBetween method to calculate the days between 01/01/2021 and the starting(checkin) date
        return start;
    }

    public static int getEnddate() throws ParseException{
        System.out.println("Ending date: (dd/MM/yyyy)");
        String Upperbound = input.next();
        Date UB = myFormat.parse(Upperbound);
        int end = (int)daysBetween(UB,first); // Call the daysBetween method to calculate the days between the 01/01/2021 and the ending(checkout) date
        return end;
    }
    ...
    public static void Add() throws ParseException { 
        ...
        if (startDay < endDay) { // check if the date range is set properly
            bookings.checkAvailability(startDay,endDay); // Call checkAvailability method in the Booking class to check which room type is available in the date range.
